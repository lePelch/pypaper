...
from . import image, monitor, paths, theme
...
class WallpaperWindow(QtWidgets.QWidget):
    def __init__(self):
        ...
        self._data_root = paths.data_root()
        self._theme_root = paths.theme_root(self._data_root)
        self._loaded_dir = paths.loaded_dir(self._data_root)
        self._state_path = paths.state_path(self._data_root)

        self._data_root.mkdir(parents=True, exist_ok=True)
        self._theme_root.mkdir(parents=True, exist_ok=True)
    ...
    def __init__(
        ...
        *,
        ...
        for row, mon in enumerate(self._monitors):
            ...
            cur = existing.get(mon)
            if isinstance(cur, int) and 1 <= cur <= self._slot_max:
                slot_box.setCurrentText(str(cur))
        ...
        self._autofill = QtWidgets.QPushButton("Autofill")
